...
> [!NOTE]
> Once this package is [published to PyPI](https://www.stainless.com/docs/guides/publish), this will become: `pip install plaza`
...
datasets = client.v1.datasets.list()
...
async def main() -> None:
    datasets = await client.v1.datasets.list()
    print(datasets.datasets)
...
```sh
# install from this staging repo
pip install 'plaza[aiohttp] @ git+ssh://git@github.com/stainless-sdks/plaza-python.git'
```
...
response = client.v1.calculate_route(
    destination={
        "lat": 0,
        "lng": 0,
    },
...
client.with_options(max_retries=5).v1.datasets.list()
```
...
client.with_options(timeout=5.0).v1.datasets.list()
```
...
dataset = response.parse()  # get the object that `v1.datasets.list()` would have returned
print(dataset.datasets)
```

These methods return an [`APIResponse`](https://github.com/stainless-sdks/plaza-python/tree/main/src/plaza/_response.py) object.

The async client returns an [`AsyncAPIResponse`](https://github.com/stainless-sdks/plaza-python/tree/main/src/plaza/_response.py) with the same structure, the only difference being `await`able methods for reading the response content.
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/plaza-python/issues) with questions, bugs, or suggestions.
